package com.mx.axeleratum.americantower.contract.documentation.config.swagger;
		...
		discoveryClient.getServices().stream().forEach(serviceId -> {
			...
			if(serviceInstances == null || serviceInstances.isEmpty()){ //Should not be the case kept for failsafe
				...
				if(jsonData.isPresent()){
					//resuelto fix no obtiene bien el json de los modulos

/*
					if (jsonData.get() instanceof Map) {
						content = (String) ((Map)jsonData.get()).get("");
					} else {
						content = getJSON(serviceId, jsonData.get());
					}
*/
